...
# Scoring weights
REASONING_LINGUISTIC_WEIGHT = env_float("HTE_LINGUISTIC_WEIGHT", 0.55)
REASONING_STATISTICAL_WEIGHT = env_float("HTE_STATISTICAL_WEIGHT", 0.45)
...
def validate_config():
    ...
    weight_sum = REASONING_LINGUISTIC_WEIGHT + REASONING_STATISTICAL_WEIGHT
    if not (0.99 <= weight_sum <= 1.01):
        errors.append(
            f"REASONING weights must sum to 1.0, got {weight_sum:.3f} "
            f"(LINGUISTIC={REASONING_LINGUISTIC_WEIGHT}, STATISTICAL={REASONING_STATISTICAL_WEIGHT})"
        )
